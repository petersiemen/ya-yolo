...
def pytorch_to_onnx(model, dummy_input, onnx_filename):
    torch.onnx.export(model,
                      ...
                      verbose=True,
                      input_names=["image"],
                      output_names=["boxes", "scores", "objectness"])
...
def convert_pytorch_to_coreml(cfg_file, class_names, state_dict_file, coreml_pipeline_filename):
    ...
    _onnx_filename = tempfile.NamedTemporaryFile(prefix='onny-yolo')
    _coreml_filename = tempfile.NamedTemporaryFile(prefix='coreml-yolo')
    _coreml_nms_filename = tempfile.NamedTemporaryFile(prefix='coreml-nms')
